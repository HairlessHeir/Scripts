...
def compareTriplets(triplet1,triplet2):
    ...
    for i in range(3):
        if triplet1.split(":")[0][i]<triplet2.split(":")[0][i]:
            ...
            return state
    if state==0:
        if (triplet1.split(":")[2])<(triplet2.split(":")[2]):
            state=-1
            return state
        elif (triplet1.split(":")[2])>(triplet2.split(":")[2]):
            state=1
            return state
...
def getArray(sortedOneTwo):
    first=[]
    second=[]
    for i in range(len(sortedOneTwo)):
        if int(sortedOneTwo[i].split(":")[1])%3==1:
            first.append(sortedOneTwo[i])
        elif int(sortedOneTwo[i].split(":")[1])%3==2:
            second.append(sortedOneTwo[i])
    field=[]
    for i in range(len(first)):
        field.append(first[i])
    for i in range(len(second)):
        field.append(second[i])
    return field 
